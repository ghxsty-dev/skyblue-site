...
const subKeys = ["logo", "social", "gaming", "discord", "creator"] as const;
        ...
        <div className="card p-6 mb-10 border-[var(--border)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {d.packages.map((pkg: any, i: number) => (
              <div key={i} className="rounded-xl border border-[var(--border)] p-4 flex flex-col">
          ...
          <a
            ...
            className="mt-5 inline-flex px-5 py-2 rounded-full bg-gradient-to-r from-[#97cdf2] to-[#59abfe] text-white text-xs font-medium hover:opacity-80 transition-opacity no-underline"
            style={{ color: "#fff" }}
          >
            {lang === "TR" ? "Satın Almak İçin Tıkla" : "Click to Buy"}
          </a>
        </div>
      </Reveal>
          ...
          <div key={key} className="mb-10">
            <Reveal delay={60 + si * 30}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base font-bold text-[var(--text)]">{sub.name}</h3>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {sub.items.map((item: any, ii: number) => (
                  <div key={ii} className="rounded-xl border border-[var(--border)] p-3 flex flex-col items-center text-center hover:border-[#59abfe] transition-all">
                    ...
                    <span className="text-xs font-extrabold bg-gradient-to-r from-[#97cdf2] to-[#59abfe] bg-clip-text text-transparent mt-1">
                      50 TL
                    </span>
                    <span className="text-[9px] text-[var(--text2)] mt-0.5">
                      {lang === "TR" ? "Sınırsız Revize" : "Unlimited Revisions"}
                    </span>
                  </div>
                ))}
